...
"""
Vitess Bank Test - Finally Validator

This script runs at end of test and checks:
...
def validate():
    ...
    log(f"[Finally] Starting validation against {config['host']}:{config['port']}")
    ...
    try:
        ...
        if not result:
            log("[Finally] No initial state found. Run first_setup.py first.")
            try:
                cursor.close()
            except Exception as close_err:
                log(f"[Finally] cursor.close() failed: {close_err}")
            try:
                conn.close()
            except Exception as conn_err:
                log(f"[Finally] conn.close() failed: {conn_err}")
            return True
        initial_num_accts, initial_total = result
        log(f"[Finally] Initial state: num_accts={initial_num_accts}, total={initial_total}")
    except Exception as e:
        log(f"[Finally] Failed to read initial_state: {e}")
        try:
            cursor.close()
        except Exception as close_err:
            log(f"[Finally] cursor.close() failed: {close_err}")
    ...
    if replica_total is not None:
        always(
            ...
            "[Finally] Replica matches primary",
